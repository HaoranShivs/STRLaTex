...
PF_TEST(EditingSystemMovesSubsections) {
  ...
  auto section =
      h.editing->Apply(h.Cmd(InsertSectionPayload{0, InlineFromText("S")}));
  h.editing->Apply(
      h.Cmd(InsertSubsectionPayload{0, 0, InlineFromText("First")}));
  h.editing->Apply(
      h.Cmd(InsertSubsectionPayload{0, 1, InlineFromText("Second")}));
...
PF_TEST(AnchorResolverAfterBlock) {
  ...
  auto p1 = h.editing->Apply(h.Cmd(
      InsertParagraphPayload{sid, std::nullopt, InlineFromText("first")}));
...
PF_TEST(EditingSystemSnapshotUndoRestoresDeletion) {
  ...
  auto p1 = h.editing->Apply(h.Cmd(InsertParagraphPayload{
      s1.created_node, std::nullopt, InlineFromText("first")}));
  auto s2 =
...
PF_TEST(EditingSystemEquationEditUndo) {
  ...
  auto s =
      h.editing->Apply(h.Cmd(InsertSectionPayload{0, InlineFromText("S")}));
  auto eq = h.editing->Apply(h.Cmd(
      InsertEquationPayload{s.created_node, std::nullopt, "E = mc^2", true}));
