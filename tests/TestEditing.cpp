...
struct TestHost {
    ...
    TestHost() {
        ...
        h.on_document_changed = [this](const DocumentChangedEvent& e) { events.push_back(e); };
        host = std::move(h);
...
PF_TEST(EditingSystemMovesSubsections) {
    TestHost h;
    auto section = h.editing->Apply(h.Cmd(InsertSectionPayload{0, InlineFromText("S")}));
    h.editing->Apply(h.Cmd(InsertSubsectionPayload{0, 0, InlineFromText("First")}));
    h.editing->Apply(h.Cmd(InsertSubsectionPayload{0, 1, InlineFromText("Second")}));
    auto moved = h.editing->Apply(h.Cmd(MoveSubsectionPayload{0, 0, 2}));
    PF_CHECK(moved.status == EditStatus::Applied);
    const auto& subsections = h.state.document().body().sections[0].subsections;
...
PF_TEST(AnchorResolverAfterBlock) {
    ...
    auto p1 = h.editing->Apply(h.Cmd(InsertParagraphPayload{sid, std::nullopt, InlineFromText("first")}));
...
PF_TEST(EditingSystemSnapshotUndoRestoresDeletion) {
    ...
    auto p1 = h.editing->Apply(h.Cmd(InsertParagraphPayload{s1.created_node, std::nullopt, InlineFromText("first")}));
    ...
    const auto* para = std::get_if<Paragraph>(&h.state.document().body().sections[0].blocks[0]);
    PF_CHECK(para != nullptr);
...
PF_TEST(EditingSystemEquationEditUndo) {
    ...
    auto eq = h.editing->Apply(h.Cmd(InsertEquationPayload{s.created_node, std::nullopt, "E = mc^2", true}));

    h.editing->Apply(h.Cmd(EditEquationPayload{eq.created_node, "F = ma", std::nullopt}));
    {
        const Document& doc = h.state.document();
...
PF_TEST(EditingSystemFigureSpanIsEditableAndUndoable) {
    ...
    InsertFigurePayload insert;
    insert.parent = s.created_node;
    insert.asset_id = AssetId("a1");
    insert.caption = InlineFromText("cap");
    auto fig = h.editing->Apply(h.Cmd(insert));
    PF_CHECK(fig.status == EditStatus::Applied);
    ...
    EditFigureSpanPayload span;
    span.figure = fig.created_node;
    span.span = FigureSpan::DoubleColumn;
    auto wide = h.editing->Apply(h.Cmd(span));
    PF_CHECK(wide.status == EditStatus::Applied);
    PF_CHECK(wide.new_revision.value > fig.new_revision.value);
    {
        const auto& block = h.state.document().body().sections[0].blocks[0];
        const auto* stored = std::get_if<Figure>(&block);
    ...
    PF_CHECK(rejected.status == EditStatus::Rejected);
}
