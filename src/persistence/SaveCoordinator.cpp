...
SaveOutcome OutcomeFromResult(const SaveResult& result, bool superseded) {
    ...
    case SaveResult::Status::Ok:
        return SaveOutcome::Saved;
    case SaveResult::Status::Queued:
        return SaveOutcome::IoError;
    case SaveResult::Status::IoError:
        return SaveOutcome::IoError;
...
const char* ToString(SaveOutcome outcome) {
    ...
    case SaveOutcome::Saved:
        return "Saved";
    case SaveOutcome::Superseded:
        return "Superseded";
    case SaveOutcome::IoError:
        return "IoError";
    case SaveOutcome::SerializeError:
        return "SerializeError";
    case SaveOutcome::Stopping:
        return "Stopping";
    }
    return "?";
}

SaveCoordinator::SaveCoordinator(std::function<void(const SaveCompletion&)> on_completed)
...
std::optional<SaveId> SaveCoordinator::Enqueue(SerializedProject snapshot, const std::filesystem::path& destination,
                                               SaveKind kind) {
    SaveTask task;
    task.kind = kind;
    ...
    const std::string id = (kind == SaveKind::Autosave ? "auto" : "") + IdGenerator::NewSaveId();
...
bool SaveCoordinator::Flush(std::chrono::milliseconds timeout) {
    ...
    return condition_.wait_for(lock, timeout, [this] { return queue_.empty() && in_flight_ == 0; });
}
...
void SaveCoordinator::WorkerLoop() {
    while (true) {
        ...
        if (task.kind == SaveKind::User && last_user_saved_revision_ && task.revision < *last_user_saved_revision_) {
            superseded = true;
        }
        SaveResult result;
        if (!superseded) {
            ...
            result.detail = "superseded by a newer save (revision " + std::to_string(task.revision.value) +
                            " < saved " + std::to_string(last_user_saved_revision_->value) + ")";
        }
...
SaveResult SaveCoordinator::RunTask(const SaveTask& task) {
    ...
    if (result.status == SaveResult::Status::Ok && task.kind == SaveKind::User) {
