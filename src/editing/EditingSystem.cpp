...
const char* ToString(EditOrigin origin) {
    switch (origin) {
        case EditOrigin::User: return "User";
        case EditOrigin::Undo: return "Undo";
        case EditOrigin::Redo: return "Redo";
        case EditOrigin::Recovery: return "Recovery";
        case EditOrigin::System: return "System";
    }
    return "Unknown";
}
...
const char* EditCommand::PayloadName() const {
    return std::visit(
        [](const auto& p) -> const char* {
            ...
            if constexpr (std::is_same_v<T, SetTitlePayload>) return "SetTitle";
            if constexpr (std::is_same_v<T, SetAbstractPayload>) return "SetAbstract";
            if constexpr (std::is_same_v<T, SetKeywordsPayload>) return "SetKeywords";
            if constexpr (std::is_same_v<T, AddAuthorPayload>) return "AddAuthor";
            if constexpr (std::is_same_v<T, SetAffiliationsPayload>) return "SetAffiliations";
            if constexpr (std::is_same_v<T, RemoveAuthorPayload>) return "RemoveAuthor";
            if constexpr (std::is_same_v<T, UpdateAuthorPayload>) return "UpdateAuthor";
            if constexpr (std::is_same_v<T, InsertSectionPayload>) return "InsertSection";
            if constexpr (std::is_same_v<T, DeleteSectionPayload>) return "DeleteSection";
            ...
            if constexpr (std::is_same_v<T, InsertEquationPayload>) return "InsertEquation";
            if constexpr (std::is_same_v<T, DeleteBlockPayload>) return "DeleteBlock";
            if constexpr (std::is_same_v<T, MoveBlockPayload>) return "MoveBlock";
            if constexpr (std::is_same_v<T, EditParagraphPayload>) return "EditParagraph";
            if constexpr (std::is_same_v<T, EditCaptionPayload>) return "EditCaption";
            if constexpr (std::is_same_v<T, EditEquationPayload>) return "EditEquation";
            if constexpr (std::is_same_v<T, InsertCitationPayload>) return "InsertCitation";
            if constexpr (std::is_same_v<T, InsertCrossReferencePayload>) return "InsertCrossReference";
            if constexpr (std::is_same_v<T, ChangeTemplatePayload>) return "ChangeTemplate";
            return "Unknown";
        },
        payload);
}
...
EditResult EditingSystem::ApplyFullPayload(const EditCommand& cmd) {
    ...
    std::optional<EditPayload> doc_payload = std::visit(
        ...
        cmd.payload);
    if (!doc_payload) {
        return EditResult::Fail(FailureReason::UnknownPayload, "unreachable payload branch");
    }
...
EditResult EditingSystem::ApplyDocumentPayload(const EditCommand& cmd,
    ...
    auto finish = [&](Result<NodeId, EditError> r) -> EditResult {
        ...
        Notify(cmd, cmd.base_revision, rev, {r.value()}, {ChangeKind::StructureChanged});
        return EditResult::Ok(rev, r.value());
    };
    auto finishVoid = [&](Result<void, EditError> r,
    ...
    if (const auto* p = std::get_if<AddAuthorPayload>(&payload)) {
        ...
        if (!r) return EditResult::Fail(FailureReason::InvalidTarget, ToString(r.error()));
        ProjectRevision rev = host_.bump_revision();
        Notify(cmd, cmd.base_revision, rev, {}, {ChangeKind::MetadataChanged});
        return EditResult::Ok(rev);
    }
    ...
    if (const auto* p = std::get_if<DeleteSectionPayload>(&payload)) {
        return finishVoid(editor.DeleteSection(p->index), {}, ChangeKind::StructureChanged);
    }
    if (const auto* p = std::get_if<MoveSectionPayload>(&payload)) {
        return finishVoid(editor.MoveSection(p->from, p->to), {}, ChangeKind::StructureChanged);
    }
    ...
    if (const auto* p = std::get_if<InsertTablePayload>(&payload)) {
        auto table = DocumentEditor::MakeTable(p->columns, p->rows, p->has_header_row);
        if (!table) {
            return EditResult::Fail(FailureReason::ConstraintViolation, ToString(table.error()));
        }
    ...
    if (const auto* p = std::get_if<MoveBlockPayload>(&payload)) {
        return finishVoid(editor.MoveBlock(p->node, p->new_parent, p->new_index), {p->node},
                          ChangeKind::StructureChanged);
    }
    if (const auto* p = std::get_if<EditParagraphPayload>(&payload)) {
        return finishVoid(editor.SetParagraphContent(p->paragraph, p->content), {p->paragraph},
    ...
    if (const auto* p = std::get_if<InsertCitationPayload>(&payload)) {
        ...
        if (!block) return EditResult::Fail(FailureReason::InvalidTarget, "paragraph missing");
        auto* para = std::get_if<Paragraph>(block);
        if (!para) return EditResult::Fail(FailureReason::InvalidTarget, "not a paragraph");
        Citation citation;
        ...
        if (p->at_index && *p->at_index <= content.size()) {
            content.insert(content.begin() + *p->at_index, InlineNode(std::move(new_cit)));
        } else {
            content.push_back(InlineNode(std::move(new_cit)));
        }
        return finishVoid(editor.SetParagraphContent(p->paragraph, content), {p->paragraph},
    ...
    if (const auto* p = std::get_if<InsertCrossReferencePayload>(&payload)) {
        ...
        InlineContent content = para->content;
        if (p->at_index && *p->at_index <= content.size()) {
            content.insert(content.begin() + *p->at_index, InlineNode(std::move(ref)));
        } else {
...
EditResult EditingSystem::Undo() {
    ...
    if (!entry) return EditResult::Fail(FailureReason::InvalidTarget, "nothing to undo");
    ...
    return EditResult::Fail(FailureReason::UnknownPayload, "unknown history entry");
}

EditResult EditingSystem::Redo() {
    auto entry = history_.PopRedo();
    if (!entry) return EditResult::Fail(FailureReason::InvalidTarget, "nothing to redo");
    if (const auto* snap = std::get_if<SnapshotHistoryAction>(&entry->action)) {
        Document& doc = host_.document();
...
void EditingSystem::Notify(const EditCommand& cmd, ProjectRevision old_rev,
                           ProjectRevision new_rev, std::vector<NodeId> affected,
                           std::vector<ChangeKind> kinds) {
    if (!host_.on_document_changed) return;
