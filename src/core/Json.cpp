...
class Parser {
  public:
    ...
    explicit Parser(const std::string& text, std::string* error, const JsonParseLimits& limits)
        : text_(text), error_(error), limits_(limits) {}

    std::unique_ptr<JsonValue> Parse() {
        try {
            if (text_.size() > limits_.max_input_bytes) {
                Fail("input exceeds maximum size (" + std::to_string(limits_.max_input_bytes) + " bytes)");
            }
            SkipWs();
            ...
            // 一次干净的解析失败，绝不能导致进程终止。
            if (error_)
                *error_ = "out of memory while parsing";
            return nullptr;
        }
    }
  ...
  private:
    ...
    void NoteNode() {
        ...
        if (node_count_ > limits_.max_nodes) {
            Fail("node count exceeds limit (" + std::to_string(limits_.max_nodes) + ")");
        }
    }
    ...
    std::unique_ptr<JsonValue> ParseValue() {
        ...
        case '[':
            return ParseArray();
        case '"':
            return std::make_unique<JsonValue>(ParseString());
        case 't':
            ExpectLiteral("true");
            NoteNode();
      ...
      public:
        DepthGuard(Parser& parser, std::size_t& depth) : parser_(parser), depth_(depth) {
            ...
            if (depth_ > parser_.limits_.max_depth) {
                parser_.Fail("nesting depth exceeds limit (" + std::to_string(parser_.limits_.max_depth) + ")");
            }
        }
        ~DepthGuard() {
            --depth_;
    ...
    std::string ParseString() {
        ...
        while (true) {
            ...
            if (out.size() > limits_.max_string_bytes) {
                Fail("string exceeds maximum length (" + std::to_string(limits_.max_string_bytes) + " bytes)");
            }
        }
        return out;
    }
