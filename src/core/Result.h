...
template <typename E> struct Unexpected2 {
...
template <typename E> E ToStringError(const std::string& value) {
    if constexpr (std::is_constructible_v<E, const std::string&>) {
        return E(value);
    } else {
        static_assert(sizeof(E) == 0, "No ToStringError specialization for this error type");
        return E{};
    }
}
