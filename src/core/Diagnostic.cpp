...
const char* ToString(DiagnosticSource source) {
    switch (source) {
    case DiagnosticSource::Document:
        return "Document";
    case DiagnosticSource::Template:
        return "Template";
    case DiagnosticSource::Bibliography:
        return "Bibliography";
    case DiagnosticSource::Renderer:
        return "Renderer";
    case DiagnosticSource::Compiler:
        return "Compiler";
    case DiagnosticSource::Persistence:
        return "Persistence";
    case DiagnosticSource::Asset:
        return "Asset";
    case DiagnosticSource::Validation:
        return "Validation";
    }
    return "Unknown";
}
...
std::string Diagnostic::Summary() const {
    ...
    out += "] ";
    out += message;
    if (location.kind == DiagnosticLocationKind::Node || location.kind == DiagnosticLocationKind::Table) {
        out += " (node ";
        ...
        out += " (" + location.file + ":" + std::to_string(location.line.value_or(0)) + ")";
    }
    // 已映射的编译器错误同时携带两种身份：它所属的 block，以及它来源的
    // 生成源码位置（方案 §17）。
    if (location.kind != DiagnosticLocationKind::GeneratedFile && location.has_file_location()) {
        out += " [" + location.file + ":" + std::to_string(location.line.value_or(0)) + "]";
    }
    return out;
}
