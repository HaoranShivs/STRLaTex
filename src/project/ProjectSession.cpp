...
ProjectSession::ProjectSession(Config config)
  ...
  texlive_root_ =
      RuntimeManager(install_root).Initialize().texlive_root;
...
bool ProjectSession::WaitForApplicationEvent(
    std::chrono::milliseconds timeout) {
  {
    std::unique_lock<std::mutex> lock(events_mutex_);
    events_condition_.wait_for(lock, timeout,
...
bool ProjectSession::OpenProject(const std::filesystem::path &project_dir,
  ...
  state_.mutable_template() =
      sp.template_id.empty() ? "generic-article" : sp.template_id;
...
bool ProjectSession::OpenProjectWithRecovery(
    const std::filesystem::path &project_dir, std::string *error,
    bool *recovered) {
  if (recovered)
    *recovered = false;
...
EditResult
ProjectSession::InsertFigureFromSource(const std::filesystem::path &source,
                                       const NodeId &parent,
