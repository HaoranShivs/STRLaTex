...
DocumentEditor::SetAbstract(const std::optional<InlineContent> &abstract_text) {
...
Result<void, EditError>
DocumentEditor::SetKeywords(const std::vector<std::string> &keywords) {
...
DocumentEditor::AddAffiliation(const std::string &name, AffiliationId id) {
  if (id.empty())
...
Result<void, EditError>
DocumentEditor::RemoveAffiliation(const AffiliationId &id) {
...
Result<void, EditError> DocumentEditor::UpdateAuthor(size_t index,
                                                     const Author &author) {
  auto &authors = document_.front_matter().authors;
  if (index >= authors.size())
    return Unexpected(ToString(EditError::InvalidTarget));
...
Result<NodeId, EditError> DocumentEditor::InsertSubsection(size_t section_index,
                                                           size_t index,
                                                           InlineContent title,
                                                           NodeId id) {
  auto &sections = document_.body().sections;
...
Result<NodeId, EditError>
DocumentEditor::InsertSubsectionAfter(const NodeId &anchor, InlineContent title,
                                      NodeId id) {
  if (id.empty())
    id = IdGenerator::NewNode();

  const auto place =
      [&](Section &section, size_t subsection_index,
...
Result<void, EditError>
DocumentEditor::DeleteSubsection(size_t section_index,
...
Result<NodeId, EditError>
DocumentEditor::InsertSubsubsectionAfter(const NodeId &anchor,
                                         InlineContent title, NodeId id) {
  if (id.empty())
    id = IdGenerator::NewNode();

  const auto place =
      [&](Subsection &sub, size_t subsubsection_index,
          std::vector<Block> taken) -> Result<NodeId, EditError> {
    Subsubsection subsub;
    subsub.id = id;
    subsub.title = std::move(title);
...
Result<void, EditError>
DocumentEditor::MoveSubsubsection(size_t section_index, size_t subsection_index,
  ...
  subsubs.insert(subsubs.begin() + static_cast<long>(insert_at),
                 std::move(moved));
  document_.BumpVersion();
  return {};
}

Result<void, EditError> DocumentEditor::MoveSubsection(size_t section_index,
                                                       size_t from, size_t to) {
  auto &sections = document_.body().sections;
  if (section_index >= sections.size())
    return Unexpected(ToString(EditError::InvalidTarget));
...
Result<void, EditError>
DocumentEditor::RenameSection(const NodeId &id, const InlineContent &title) {
...
Result<void, EditError>
DocumentEditor::RenameSubsection(const NodeId &id, const InlineContent &title) {
  if (auto *sub = FindSubsection(id)) {
    sub->title = title;
    document_.BumpVersion();
    return {};
...
Result<void, EditError>
DocumentEditor::RenameSubsubsection(const NodeId &id,
...
std::optional<DocumentEditor::BlockPosition>
DocumentEditor::FindBlockPosition(const NodeId &id) {
  auto address = LocateNode(document_, id);
...
Result<void, EditError>
DocumentEditor::MoveBlock(const NodeId &id, const NodeId &new_parent,
  ...
  dst_blocks->insert(dst_blocks->begin() + static_cast<long>(at),
                     std::move(moved));
  document_.BumpVersion();
  return {};
}
...
Result<void, EditError>
DocumentEditor::SetCaption(const NodeId &id, const InlineContent &caption) {
  Block *block = FindBlock(id);
  if (!block)
    return Unexpected(ToString(EditError::NotFound));
...
Result<void, EditError>
DocumentEditor::SetEquationSource(const NodeId &id, const std::string &source,
...
Result<void, EditError>
DocumentEditor::SetFigureAsset(const NodeId &id, const AssetId &asset_id,
...
Result<void, EditError>
DocumentEditor::SetFigureAltText(const NodeId &id, const std::string &alt) {
  Block *block = FindBlock(id);
  if (!block)
    return Unexpected(ToString(EditError::NotFound));
  if (auto *fig = std::get_if<Figure>(block)) {
...
Result<void, EditError> DocumentEditor::SetFigureSpan(const NodeId &id,
                                                      FigureSpan span) {
  Block *block = FindBlock(id);
  if (!block)
    return Unexpected(ToString(EditError::NotFound));
...
std::vector<Block> *
DocumentEditor::FindBlockListForParent(const NodeId &parent) {
...
Result<Table, EditError>
DocumentEditor::MakeTable(std::vector<TableColumn> columns, size_t rows,
                          bool has_header_row, NodeId id) {
  if (columns.empty())
...
Result<void, EditError> DocumentEditor::InsertTableRow(const NodeId &id,
                                                       size_t index) {
  Table *table = [&]() -> Table * {
    Block *block = FindBlock(id);
...
Result<void, EditError> DocumentEditor::DeleteTableRow(const NodeId &id,
                                                       size_t index) {
  Table *table = [&]() -> Table * {
    Block *block = FindBlock(id);
    return block ? std::get_if<Table>(block) : nullptr;
...
Result<void, EditError>
DocumentEditor::InsertTableColumn(const NodeId &id, size_t index,
...
Result<void, EditError> DocumentEditor::DeleteTableColumn(const NodeId &id,
                                                          size_t index) {
  Table *table = [&]() -> Table * {
    Block *block = FindBlock(id);
    return block ? std::get_if<Table>(block) : nullptr;
...
Result<void, EditError>
DocumentEditor::SetTableCell(const NodeId &id, size_t row, size_t column,
                             const InlineContent &content) {
  Table *table = [&]() -> Table * {
    Block *block = FindBlock(id);
    return block ? std::get_if<Table>(block) : nullptr;
