...
class DocumentEditor {
  public:
    ...
    Result<AffiliationId, EditError> AddAffiliation(const std::string& name, AffiliationId id = AffiliationId{});
    ...
    // 方式与显式 index 控制。
    Result<NodeId, EditError> InsertSection(size_t index, InlineContent title, NodeId id = NodeId{});
    Result<void, EditError> DeleteSection(size_t index);
    Result<void, EditError> MoveSection(size_t from, size_t to);

    Result<NodeId, EditError> InsertSubsection(size_t section_index, size_t index, InlineContent title,
                                               NodeId id = NodeId{});
    ...
    Result<NodeId, EditError> InsertSubsectionAfter(const NodeId& anchor, InlineContent title, NodeId id = NodeId{});
    ...
    Result<NodeId, EditError> InsertSubsubsection(size_t section_index, size_t subsection_index, size_t index,
                                                  InlineContent title, NodeId id = NodeId{});
    // 按阅读顺序在 `anchor` 之后插入；anchor 之后的 blocks 移入新建的
    // subsubsection，与 InsertSubsectionAfter 行为一致。
    Result<NodeId, EditError> InsertSubsubsectionAfter(const NodeId& anchor, InlineContent title, NodeId id = NodeId{});
    Result<void, EditError> DeleteSubsubsection(size_t section_index, size_t subsection_index,
                                                size_t subsubsection_index);
    Result<void, EditError> MoveSubsubsection(size_t section_index, size_t subsection_index, size_t from, size_t to);
    ...
    // Block 变更
    Result<NodeId, EditError> InsertBlock(const NodeId& parent, std::optional<size_t> index, Block block,
                                          NodeId id = NodeId{});
    Result<void, EditError> DeleteBlock(const NodeId& id);
    Result<void, EditError> MoveBlock(const NodeId& id, const NodeId& new_parent, std::optional<size_t> new_index);
    ...
    // Table 变更——强制维持矩形不变量。
    static Result<Table, EditError> MakeTable(std::vector<TableColumn> columns, size_t rows, bool has_header_row,
                                              NodeId id = NodeId{});
    ...
    Result<void, EditError> InsertTableColumn(const NodeId& id, size_t index, ColumnAlignment alignment);
    Result<void, EditError> DeleteTableColumn(const NodeId& id, size_t index);
    Result<void, EditError> SetTableCell(const NodeId& id, size_t row, size_t column, const InlineContent& content);
