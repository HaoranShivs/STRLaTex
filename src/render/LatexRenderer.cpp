...
std::string LatexRenderer::AssetPathFor(const AssetId& id) const {
    ...
    // 以便扩展名能传递给编译器。
    const std::string file = it != asset_files_.end() ? it->second : id.value() + ".img";
    return "assets/" + file;
}
...
void LatexRenderer::RenderBlock(const Block& block, std::string* out, SourceMap* smap) const {
    auto start_line = static_cast<std::uint32_t>(1 + std::count(out->begin(), out->end(), '\n'));
    ...
    if (const auto* para = std::get_if<Paragraph>(&block)) {
        ...
        *out += std::string("\\begin{") + open_env + "}[htbp]\n\\centering\n" + "\\includegraphics[width=" + buf +
                "\\linewidth]{" + asset_file + "}\n";
        if (!caption.empty()) {
            *out += "\\caption{" + caption + "}";
            if (!alt.empty())
        ...
        *out += GenerateDisplayMath(eq->expression, eq->numbered, eq->numbered ? LabelFor(eq->id) : std::string());
    }

    auto end_line = static_cast<std::uint32_t>(1 + std::count(out->begin(), out->end(), '\n'));
...
RenderResult LatexRenderer::Render(const RenderRequest& request) const {
    ...
    if (!fm.authors.empty()) {
        const auto ordinal = [](size_t index) -> std::string {
            static const char* kSupers[] = {"1", "2", "3", "4", "5", "6", "7", "8", "9"};
            if (index < 9) {
        ...
        for (size_t i = 0; i < fm.authors.size(); ++i) {
            ...
            for (const auto& aff_id : author.affiliations) {
                ...
                if (slot > 0 && std::find(slots.begin(), slots.end(), slot) == slots.end()) {
    ...
    for (const auto& section : doc.body().sections) {
        ...
        for (const auto& sub : section.subsections) {
            ...
            for (const auto& subsub : sub.subsubsections) {
                ...
                tex += "\\subsubsection{" + subsub_title + "}\\label{" + subsub.id.value() + "}\n\n";
