...
class DiagnosticMapper {
  public:
    std::vector<Diagnostic> Map(const CompileResult& result, const SourceMap& source_map, ProjectRevision revision,
    ...
    static std::string CodeFor(const CompilerMessage& message, DiagnosticSeverity* severity);
};
