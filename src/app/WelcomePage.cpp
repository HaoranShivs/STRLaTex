...
WelcomePage::WelcomePage(QWidget* parent) : QWidget(parent) {
    ...
    title->setStyleSheet(
        QString("font-size: 26pt; font-weight: 700; color: %1; background: transparent;").arg(theme::kPrimaryText));
    layout->addWidget(title);

    auto* tagline = new QLabel("Create academic papers without writing LaTeX.", this);
    tagline->setStyleSheet(QString("font-size: 12pt; color: %1; background: transparent; margin-bottom: 24px;")
    ...
    connect(new_btn, &QPushButton::clicked, this, &WelcomePage::NewProjectRequested);
    layout->addWidget(new_btn);

    auto* open_btn = new QPushButton("Open Project…", this);
    open_btn->setFixedWidth(220);
    connect(open_btn, &QPushButton::clicked, this, &WelcomePage::OpenProjectRequested);
    ...
    recent_list_->setStyleSheet(QString("QListWidget { background: transparent; border: none; }"
                                        "QListWidget::item { padding: 8px; border-radius: 6px; }"
                                        "QListWidget::item:hover { background: %1; }")
                                    .arg(theme::kAccentSoft));
    connect(recent_list_, &QListWidget::itemClicked, this,
            [this](QListWidgetItem* item) { emit RecentActivated(item->data(Qt::UserRole).toString()); });
