...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent) {
    ...
    connect(controller_, &ProjectController::documentChanged, this, &MainWindow::RefreshDocumentView);
    connect(editor_, &BlockEditor::RowCommitted, this, [this]() {
        if (pending_structural_refresh_)
            RefreshDocumentView();
    });
    connect(controller_, &ProjectController::previewUpdated, this, &MainWindow::OnPreviewUpdated);
    connect(controller_, &ProjectController::saveFinished, this, &MainWindow::OnSaveFinished);
    // P0-06：由权威的 persistence/preview/revision 投影驱动状态标签；
    // UI 中不保留第二份 dirty 标志。
    connect(controller_, &ProjectController::stateChanged, this, &MainWindow::RenderProjectState);
    // Problems 与 Build Log 直接消费结构化流水线（Build Diagnostics 方案
    // §37）：MainWindow 只负责连接各组件，绝不解析日志、计算源码映射或
    // 创建 Diagnostic。
    connect(controller_, &ProjectController::buildCompleted, this, &MainWindow::OnBuildCompleted);
    connect(controller_, &ProjectController::buildEvent, this, &MainWindow::OnBuildEvent);
    connect(problems_, &ProblemsPanel::DiagnosticActivated, this,
            [this](const pf::Diagnostic& diagnostic) { OnProblemActivated(diagnostic); });
    connect(controller_, &ProjectController::buildStatusChanged, this, &MainWindow::OnBuildStatusChanged);
...
void MainWindow::BuildUi() {
    ...
    connect(welcome_, &WelcomePage::NewProjectRequested, this, &MainWindow::OnNewProject);
    connect(welcome_, &WelcomePage::OpenProjectRequested, this, &MainWindow::OnOpenProject);
    ...
    editor_->SetAssetPathResolver([this](const AssetId& id) {
        ...
        auto resolved =
            pf::ResolveUntrustedProjectPath(controller_->session().paths().assets_dir, metadata->relative_path);
        if (!resolved.ok())
            return QString();
...
QWidget* MainWindow::BuildHeader() {
    ...
    header->setStyleSheet(QString("QWidget#headerBar { background: %1; border-bottom: 1px solid %2; }")
                              .arg(theme::kEditorBackground, theme::kDivider));
    auto* layout = new QHBoxLayout(header);
    layout->setContentsMargins(16, 6, 16, 6);

    auto* logo = new QLabel("PaperForge", header);
    logo->setStyleSheet(QString("font-weight: 700; color: %1;").arg(theme::kPrimaryText));
    ...
    template_combo_->setStyleSheet(
        QString("QComboBox { border: none; background: transparent; color: %1; }").arg(theme::kSecondaryText));
    ...
    focus_button_->setToolTip("Hide the outline and PDF preview to write full width");
    focus_button_->setStyleSheet(QString("QPushButton#focusToggle { background: transparent; border: 1px "
                                         "solid %1;"
                                         ...
                                         " color: white; }")
                                     .arg(theme::kDivider, theme::kSecondaryText, theme::kAccentSoft, theme::kAccent));
    connect(focus_button_, &QPushButton::toggled, this, &MainWindow::OnToggleFocusMode);
...
void MainWindow::OnToggleFocusMode(bool on) {
    ...
    if (focus_button_) {
        focus_button_->setText(on ? QStringLiteral("⤡  Exit Focus") : QStringLiteral("⤢  Focus"));
    }
}

void MainWindow::BuildMenus() {
    QMenu* file_menu = menuBar()->addMenu("&File");
    file_menu->addAction("&New Project…", this, &MainWindow::OnNewProject, QKeySequence::New);
    file_menu->addAction("&Open Project…", this, &MainWindow::OnOpenProject, QKeySequence::Open);
    ...
    file_menu->addSeparator();
    file_menu->addAction("&Import Bibliography (.bib)…", this, &MainWindow::OnImportBibliography);
    file_menu->addSeparator();
    ...
    build_menu->addAction("&Build Now", this, &MainWindow::OnBuild, QKeySequence("Ctrl+B"));
}

void MainWindow::WireEditor() {
    connect(editor_, &BlockEditor::TitleEdited, this, [this](QString t) { controller_->SetTitle(std::move(t)); });
    connect(editor_, &BlockEditor::AuthorsEdited, this,
            [this](QString t) { controller_->SetAuthorsText(std::move(t)); });
    connect(editor_, &BlockEditor::AffiliationsEdited, this,
            [this](QString t) { controller_->SetAffiliationsText(std::move(t)); });
    connect(editor_, &BlockEditor::AbstractEdited, this, [this](QString t) { controller_->SetAbstract(std::move(t)); });
    connect(editor_, &BlockEditor::KeywordsEdited, this,
            [this](QString t) { controller_->SetKeywordsText(std::move(t)); });
    ...
    // （引用方案 §5）。
    connect(editor_, &BlockEditor::ParagraphContentEdited, this, [this](QString node, const InlineContent& content) {
        if (shutting_down_)
            return;
    ...
    connect(editor_, &BlockEditor::EquationEdited, this,
            [this](QString node, QString math, bool numbered, QString label) {
                controller_->EditEquation(NodeId(node.toStdString()), std::move(math), numbered, std::move(label));
            });
    connect(editor_, &BlockEditor::SectionRenamed, this, [this](QString node, QString text) {
        controller_->RenameSection(NodeId(node.toStdString()), std::move(text));
    });
    connect(editor_, &BlockEditor::SubsectionRenamed, this, [this](QString node, QString text) {
        controller_->RenameSubsection(NodeId(node.toStdString()), std::move(text));
    });
    connect(editor_, &BlockEditor::SubsubsectionRenamed, this, [this](QString node, QString text) {
        controller_->RenameSubsubsection(NodeId(node.toStdString()), std::move(text));
    });
    connect(editor_, &BlockEditor::AuthorAffiliationToggled, this,
            [this](int author_index, QString affiliation, bool linked) {
                if (shutting_down_)
                    return;
                const auto result = controller_->SetAuthorAffiliation(static_cast<size_t>(author_index),
    ...
    connect(editor_, &BlockEditor::MoveBlockToRequested, this, [this](QString node, QString anchor) {
        if (shutting_down_)
            return;
        const auto result = controller_->MoveNodeAfter(NodeId(node.toStdString()), NodeId(anchor.toStdString()));
        if (result.status != pf::EditStatus::Applied) {
            statusBar()->showMessage(ToQ(result.detail), 3000);
        }
    });
    connect(editor_, &BlockEditor::InsertBlockRequested, this, [this](QString type, QString after) {
        if (after.isEmpty()) {
            // 没有锚点：正文尚无任何块，此时唯一有意义的插入
            // 就是第一个 section。
            if (type == "section") {
                const EditResult created = controller_->InsertSection(QString());
                if (created.status == EditStatus::Applied && !created.created_node.empty()) {
                    editor_->RevealNode(QString::fromStdString(created.created_node.value()));
                }
            }
            return;
        }
        ...
        if (type == "section") {
            ...
            QString path = QFileDialog::getOpenFileName(this, "Insert Figure", {}, "Images (*.png *.jpg *.jpeg *.gif)");
            if (!path.isEmpty()) {
        ...
        if (result.status != pf::EditStatus::Applied) {
            statusBar()->showMessage("Could not insert block: " + ToQ(result.detail), 5000);
        }
    });
    connect(editor_, &BlockEditor::CaptionEdited, this,
            [this](QString node, QString caption) { controller_->EditCaption(NodeId(node.toStdString()), caption); });
    connect(editor_, &BlockEditor::FigureSpanChanged, this, [this](QString node, bool double_column) {
        if (shutting_down_)
            return;
        QScopedValueRollback<bool> keep_rows(editor_change_in_progress_, true);
        const auto result = controller_->EditFigureSpan(NodeId(node.toStdString()), double_column);
        if (result.status != pf::EditStatus::Applied) {
            statusBar()->showMessage(ToQ(result.detail), 3000);
        }
    });
    connect(editor_, &BlockEditor::DeleteBlockRequested, this, [this](QString node) {
        auto result = controller_->DeleteNode(NodeId(node.toStdString()));
        if (result.status != pf::EditStatus::Applied) {
            statusBar()->showMessage("Could not delete block: " + ToQ(result.detail), 5000);
        }
    });
    connect(editor_, &BlockEditor::MoveBlockRequested, this, [this](QString node, int direction) {
        auto result = controller_->MoveNode(NodeId(node.toStdString()), direction);
        if (result.status != pf::EditStatus::Applied) {
            statusBar()->showMessage(ToQ(result.detail), 3000);
        }
    });
    ...
    // 并作为 rich content 提交。
    connect(outline_, &OutlinePanel::NodeActivated, this, [this](QString node) { editor_->RevealNode(node); });
    // 反向同步（UI 方案 §10）：光标移动 → 大纲高亮所属 section，
    // 因此长稿件始终能显示你所在的位置。
    connect(editor_, &BlockEditor::FocusOutlineChanged, outline_, &OutlinePanel::SelectNode);
    connect(outline_, &OutlinePanel::CitationChosen, this, [this](QString key) {
        auto focused = editor_->FocusedNodeId();
        if (!focused) {
            statusBar()->showMessage("Click into a Text block first, then pick a reference", 4000);
            return;
        }
        ...
        if (editor_->InsertCitationIntoParagraph(*focused, key)) {
            ...
            statusBar()->showMessage("The focused block is not a text row - citations attach to "
                                     "paragraphs",
                                     4000);
        }
    });
...
void MainWindow::UpdateRequiredHints() {
    ...
    const auto* def = TemplateRegistry::Instance().Find(controller_->session().state().template_selection());
    if (def) {
        hints.title = def->required.title;
        hints.authors = def->required.authors;
        hints.affiliations = def->required.affiliations || def->required.author_affiliations;
...
void MainWindow::RefreshReferenceItems() {
    ...
    for (const auto& entry : controller_->session().bibliography().Entries()) {
        PopupList::Item item;
        QString authors = entry.authors.empty() ? QString() : ToQ(entry.authors.front()) + " et al.";
        item.label = authors.isEmpty() ? ToQ(entry.title) : authors + " · " + ToQ(entry.year);
...
int MainWindow::CountWords() const {
    ...
    return text.split(QRegularExpression(QStringLiteral(R"(\s+)")), Qt::SkipEmptyParts).size();
}
...
void MainWindow::OnNewProject() {
    ...
    QString dir = QFileDialog::getExistingDirectory(this, "New Project Directory");
    if (dir.isEmpty())
        return;
    if (std::filesystem::exists(dir.toStdString() + "/project.paper")) {
        QMessageBox::warning(this, "PaperForge", "Directory already contains a project.");
        return;
    }
    if (MaybeSaveBeforeDestructiveNavigation() == DestructiveNavigationDecision::Cancel) {
        return;
    }
    std::string error;
    if (!controller_->NewProject(dir, &error)) {
        QMessageBox::critical(this, "PaperForge",
                              "Failed to create project: " +
                                  (error.empty() ? QString("unknown error") : QString::fromStdString(error)));
        return;
    }
    QSettings settings;
...
bool MainWindow::OpenProjectDir(const QString& dir) {
    ...
    controller_->StartAutosave();
    if (recovered) {
        QMessageBox::information(this, "PaperForge", "Recovered unsaved changes from the autosave snapshot.");
    }
    ShowWorkspace(true);
...
MainWindow::DestructiveNavigationDecision MainWindow::MaybeSaveBeforeDestructiveNavigation() {
    // 固定顺序（整改方案 P0-01）：
    ...
    const auto ask = [this](const QString& text, const QString& informative, const QString& save_label,
                            const QString& discard_label, const QString& cancel_label) {
        ...
        QPushButton* discard = box.addButton(discard_label, QMessageBox::DestructiveRole);
    ...
    case PersistenceState::Dirty: {
        const int choice =
            ask("This project has unsaved changes.", "Save them before continuing?", "Save", "Discard", "Cancel");
        if (choice == 2)
        ...
        if (result.status != SaveResult::Status::Queued) {
            QMessageBox::critical(this, "PaperForge", "Could not save: " + ToQ(result.detail));
            return DestructiveNavigationDecision::Cancel;
        }
        if (!WaitForUserSaveCompletion()) {
            QMessageBox::warning(this, "PaperForge", "The save is still running. Cancel the navigation and try again.");
            return DestructiveNavigationDecision::Cancel;
        }
        RenderProjectState();
        // 只有项目现在确实为 Clean 才继续；保存失败（或被更新的编辑取代）
        // 时必须把用户留在此处。
        return session.persistence_state() == PersistenceState::Clean ? DestructiveNavigationDecision::Proceed
                                                                      : DestructiveNavigationDecision::Cancel;
    }

    case PersistenceState::Saving: {
        const int choice =
            ask("A save is still running.", "Wait for it to finish before continuing?", "Wait", "Discard", "Cancel");
        if (choice == 2)
            return DestructiveNavigationDecision::Cancel;
        if (choice == 1)
            return DestructiveNavigationDecision::Proceed;
        if (!WaitForUserSaveCompletion()) {
            QMessageBox::warning(this, "PaperForge", "The save did not finish in time.");
            return DestructiveNavigationDecision::Cancel;
        }
        RenderProjectState();
        return DestructiveNavigationDecision::Proceed;
    ...
    case PersistenceState::SaveFailed: {
        const int choice =
            ask("The last save failed.", "Retry saving before continuing?", "Retry Save", "Discard", "Cancel");
        if (choice == 2)
            return DestructiveNavigationDecision::Cancel;
        if (choice == 1)
            return DestructiveNavigationDecision::Proceed;
        ...
        if (!WaitForUserSaveCompletion()) {
            QMessageBox::warning(this, "PaperForge", "The retry did not finish in time.");
            return DestructiveNavigationDecision::Cancel;
        }
        RenderProjectState();
        return session.persistence_state() == PersistenceState::Clean ? DestructiveNavigationDecision::Proceed
...
void MainWindow::OnImportBibliography() {
    ...
    // Document（引用方案 §6/§9）。
    editor_->CommitFocused();
    QString path = QFileDialog::getOpenFileName(this, "Import BibTeX", {}, "BibTeX (*.bib)");
    if (path.isEmpty())
        return;
    QFile file(path);
    ...
    if (result.status == BibliographyImportResult::Status::Ok) {
        QString message = QString("Bibliography imported: %1 entries from %2").arg(result.entry_count).arg(path);
...
void MainWindow::RefreshDocumentView() {
    ...
    if (const auto* tpl = TemplateRegistry::Instance().Find(controller_->session().state().template_selection())) {
...
void MainWindow::OnPreviewUpdated(const pf::PreviewUpdate& update) {
    ...
    if (update.success && update.pdf.valid()) {
        ...
        build_button_->setStyleSheet(QString("QPushButton { background: %1; color: white; border: none;"
                                             " font-weight: 600; }")
                                         .arg(theme::kOk));
...
void MainWindow::OnBuildCompleted(const pf::BuildResult& result) {
    ...
    if (result.outcome == BuildResult::Outcome::Failure) {
        const bool system_failure = result.failure_kind == CompileFailureKind::RuntimeMissing ||
...
void MainWindow::OnProblemActivated(const pf::Diagnostic& diagnostic) {
    ...
    if (diagnostic.location.has_block_location()) {
        ...
        statusBar()->showMessage("Location unavailable - that block is gone; rebuild to refresh "
                                 "the problems list",
                                 5000);
        return;
    }
    if (diagnostic.location.has_file_location() || !diagnostic.raw_message.empty()) {
        problems_->ShowBuildLog();
    }
}
...
void MainWindow::OnBuildStatusChanged(const QString& status) {
    ...
    if (status == "Idle") {
        building_ = false;
        if (build_button_->text() == "Cancel  \u25A0" || build_button_->text() == "Cancelling \u25EF" ||
...
void MainWindow::ZoomPreviewForTest(double zoom, double scroll_x, double scroll_y) {
