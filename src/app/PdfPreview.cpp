...
int QueryPageCount(const QString& pdf_path) {
    ...
    for (const QString& line : output.split(QLatin1Char('\n'))) {
        if (line.startsWith(QLatin1String("Pages:"))) {
            bool ok = false;
            const int count =
                line.mid(QStringLiteral("Pages:").size()).trimmed().toInt(&ok);
...
QPixmap RenderPageToPixmap(const QString& pdf_path, int page, int dpi,
    ...
    process.start("pdftoppm",
                  {"-png", "-r", QString::number(dpi), "-f",
                   QString::number(page), "-l", QString::number(page),
                   pdf_path, dir.path() + "/page"});
    ...
    for (const char* form : {"-%1.png", "-%01.png", "-%02.png",
        ...
        const QString candidate =
            prefix + QString::fromLatin1(form).arg(page);
    ...
    if (page == 1 && width_pt && height_pt) {
        ...
        *height_pt =
            pixmap.height() / static_cast<double>(dpi) * kPointsPerInch;
    }
    return pixmap;
}
...
PdfPreview::PdfPreview(QWidget* parent) : QWidget(parent) {
    ...
    header->setStyleSheet(
        QString("QWidget#pdfPreviewHeader { background: %1;"
    ...
    page_indicator_->setStyleSheet(
        QString("color: %1; font-size: 8pt; margin-left: 6px;")
    ...
    zoom_label_->setStyleSheet(
        QString("color: %1; font-size: 8pt;").arg(theme::kSecondaryText));
    ...
    scroll_->setStyleSheet(
        QString("QScrollArea { background: %1; }").arg(theme::kSidePanel));
    scroll_->viewport()->installEventFilter(this);

    column_ = new QWidget(scroll_);
    column_->setObjectName("pdfPageColumn");
    column_->setStyleSheet(
        QString("QWidget#pdfPageColumn { background: %1; }")
...
void PdfPreview::SetDocument(const QString& pdf_path) {
    ...
    if (probe.isNull()) {
        ShowMessage(QStringLiteral(
                        "PDF ready but page rendering unavailable.\n") +
                    pdf_path);
        return;
    }
...
void PdfPreview::ShowMessage(const QString& text) {
    ...
    if (message_label_ == nullptr) {
        ...
        message_label_->setStyleSheet(
            QString("background: transparent; color: %1; padding: 24px;")
                .arg(theme::kSecondaryText));
    }
    message_label_->setText(text);
    message_label_->setFixedWidth(
...
void PdfPreview::SetMessage(const QString& text) { ShowMessage(text); }
...
QSize PdfPreview::PageSizeAtZoom() const {
    const int width = std::max(
        1, static_cast<int>(std::lround(page_width_pt_ / kPointsPerInch *
                                        kBaselineDpi * zoom_)));
    const int height = std::max(
        1, static_cast<int>(std::lround(page_height_pt_ / kPointsPerInch *
                                        kBaselineDpi * zoom_)));
...
int PdfPreview::RenderDpiFor(double zoom) const {
    const int dpi = static_cast<int>(std::lround(
        kBaselineDpi * zoom * qApp->devicePixelRatio()));
...
void PdfPreview::RenderPage(int index, int dpi) {
    ...
    const QPixmap fresh =
        RenderPageToPixmap(pdf_path_, index + 1, dpi, nullptr, nullptr);
    if (fresh.isNull()) return;
    slot.pixmap = fresh;
    slot.pixmap_dpi = dpi;
    if (slot.label) {
        slot.label->setText(QString());
        slot.label->setPixmap(fresh.scaled(PageSizeAtZoom(),
                                           Qt::KeepAspectRatio,
...
void PdfPreview::ReleaseFarPages(int first, int last) {
    for (int i = 0; i < static_cast<int>(pages_.size()); ++i) {
        ...
        if (slot.pixmap.isNull()) continue;
        slot.pixmap = QPixmap();
        slot.pixmap_dpi = 0;
        if (slot.label) {
...
void PdfPreview::EnsureVisiblePages() {
    ...
    const int first = std::clamp(top / std::max(1, stride), 0,
                                 page_count_ - 1);
    const int last = std::clamp((top + viewport) / std::max(1, stride), 0,
                                page_count_ - 1);
    ...
    page_indicator_->setText(QStringLiteral("p. %1 / %2")
                                 .arg(first + 1)
                                 .arg(page_count_));
}
...
void PdfPreview::UpdateColumnSize() {
    ...
    if (pages_.empty()) {
        const int height =
            message_label_ && message_label_->isVisible()
                ? message_label_->sizeHint().height()
                : 0;
    ...
    column_->resize(std::max(page.width(), viewport_width),
...
void PdfPreview::UpdateZoomLabel() {
    zoom_label_->setText(
        QString::number(static_cast<int>(std::lround(zoom_ * 100))) + "%");
}
...
void PdfPreview::ZoomIn() { SetZoom(zoom_ * 1.25); }
void PdfPreview::ZoomOut() { SetZoom(zoom_ / 1.25); }
...
void PdfPreview::ScrollTo(double fraction_x, double fraction_y) {
    ...
    horizontal->setValue(static_cast<int>(
        std::lround(horizontal->maximum() * std::clamp(fraction_x, 0.0, 1.0))));
    vertical->setValue(static_cast<int>(
        std::lround(vertical->maximum() * std::clamp(fraction_y, 0.0, 1.0))));
    EnsureVisiblePages();
}
...
void PdfPreview::ZoomAtPoint(double new_zoom, const QPoint& viewport_pos) {
    ...
    horizontal->setValue(
        static_cast<int>(std::lround(fx * new_w - viewport_pos.x())));
    vertical->setValue(
        static_cast<int>(std::lround(fy * new_h - viewport_pos.y())));
    EnsureVisiblePages();
}
...
bool PdfPreview::eventFilter(QObject* watched, QEvent* event) {
    ...
    switch (event->type()) {
        ...
        case QEvent::MouseMove: {
            ...
            pan_origin_ = mouse->pos();
            scroll_->horizontalScrollBar()->setValue(
                scroll_->horizontalScrollBar()->value() - delta.x());
            scroll_->verticalScrollBar()->setValue(
                scroll_->verticalScrollBar()->value() - delta.y());
            return true;
        }
