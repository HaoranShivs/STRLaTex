...
ProjectController::ProjectController(QObject* parent) : QObject(parent) {
    ...
    config.workspace_root = std::filesystem::temp_directory_path() / "paperforge-gui-builds";
    ...
    session_->SetBuildResultHandler([this](const BuildResult& result) { emit buildCompleted(result); });
    // 当前 build 的实时构建日志事件（session 已丢弃所有 build id 失效的事件）。
    session_->SetBuildEventHandler([this](const BuildEvent& event) { emit buildEvent(event); });
    session_->SetPreviewUpdateHandler([this](const PreviewUpdate& update) { emit previewUpdated(update); });
...
std::optional<ProjectController::InsertionPoint> ProjectController::ResolveInsertionPoint(const NodeId& anchor) const {
    ...
    for (const auto& section : sections) {
        ...
        for (size_t i = 0; i < section.blocks.size(); ++i) {
            const NodeId id = std::visit([](const auto& block) { return block.id; }, section.blocks[i]);
            if (id == anchor)
        ...
        for (const auto& subsection : section.subsections) {
            ...
            for (size_t i = 0; i < subsection.blocks.size(); ++i) {
                const NodeId id = std::visit([](const auto& block) { return block.id; }, subsection.blocks[i]);
                if (id == anchor) {
...
bool ProjectController::OpenProjectWithRecovery(const QString& dir, bool* recovered) {
    std::string error;
...
void ProjectController::StartAutosave() {
    session_->StartAutosaveTimer();
}

void ProjectController::StopAutosave() {
    session_->StopAutosaveTimer();
}
...
void ProjectController::EmitProjectStateChanged() {
    ...
    emit stateChanged(ToString(session_->persistence_state()), ToString(session_->preview_state()),
...
EditResult ProjectController::SetAuthorsText(const QString& comma_separated) {
    ...
    // 因此编辑姓名绝不会丢失该关联。
    QStringList names =
        comma_separated.split(QRegularExpression(QStringLiteral(R"([,\n\x{00B7}]+)")), Qt::SkipEmptyParts);
    std::vector<Author> authors;
    const auto& affiliations = session_->state().document().front_matter().affiliations;
    for (const auto& raw : names) {
        ...
        if (match.hasMatch()) {
            for (const QChar& marker : match.captured(1)) {
                ...
                if (ordinal >= 1 && static_cast<size_t>(ordinal) <= affiliations.size()) {
                    author.affiliations.push_back(affiliations[static_cast<size_t>(ordinal) - 1].id);
                }
            }
...
EditResult ProjectController::SetAuthorAffiliation(size_t author_index, const AffiliationId& affiliation, bool linked) {
    const auto& authors = session_->state().document().front_matter().authors;
    if (author_index >= authors.size()) {
        return EditResult::Fail(FailureReason::InvalidTarget, "author index out of range");
    }
    ...
    if (linked) {
        ...
        if (ordered.size() != links.size() + 1) {
            return EditResult::Fail(FailureReason::InvalidTarget, "unknown institution");
        }
...
EditResult ProjectController::SetAffiliationsText(const QString& semicolon_separated) {
    SetAffiliationsPayload p;
    QStringList names = semicolon_separated.split(QRegularExpression(QStringLiteral(R"([;\n]+)")), Qt::SkipEmptyParts);
    const auto& existing = session_->state().document().front_matter().affiliations;
    size_t slot = 0;
    for (const auto& raw : names) {
        QString name = raw.trimmed();
        // 去掉开头的序号标记，使 "1 University" 与上标形式
        // 都解析为同一机构。
        static const QRegularExpression leading(QStringLiteral(R"(^[\x{00B9}\x{00B2}\x{00B3}\x{2070}-\x{209F}]+\s*)"));
        name.remove(leading);
        if (name.isEmpty())
            continue;
        Affiliation aff;
        ...
        // 因此每敲一次键就生成新 id 会
        // 悄悄使每位作者脱离其机构。
        aff.id = slot < existing.size() ? existing[slot].id : AffiliationId(IdGenerator::NewAffiliationId());
        aff.name = ToStd(name);
...
EditResult ProjectController::InsertSectionAfter(const NodeId& anchor, const QString& title) {
    const auto& sections = session_->state().document().body().sections;
    for (size_t si = 0; si < sections.size(); ++si) {
        ...
        for (const auto& block : sections[si].blocks) {
            if (belongs)
                break;
            belongs = std::visit([&](const auto& value) { return value.id == anchor; }, block);
        }
...
EditResult ProjectController::InsertSubsection(size_t section_index, const QString& title) {
    InsertSubsectionPayload p;
    p.section_index = section_index;
    p.index = session_->state().document().body().sections[section_index].subsections.size();
    p.title = InlineFromText(ToStd(title));
    auto r = session_->Execute(MakeCmd(std::move(p)));
    if (r.status == EditStatus::Applied)
        EmitDocumentChanged();
...
EditResult ProjectController::InsertSubsectionAfter(const NodeId& anchor, const QString& title) {
...
EditResult ProjectController::InsertSubsubsection(size_t section_index, size_t subsection_index, const QString& title) {
    InsertSubsubsectionPayload p;
    p.section_index = section_index;
    p.subsection_index = subsection_index;
    const auto& subsubsections =
        session_->state().document().body().sections[section_index].subsections[subsection_index].subsubsections;
...
EditResult ProjectController::InsertSubsubsectionAfter(const NodeId& anchor, const QString& title) {
...
EditResult ProjectController::RenameSubsubsection(const NodeId& subsubsection, const QString& title) {
...
EditResult ProjectController::DeleteSubsubsection(const NodeId& subsubsection) {
    ...
    if (!address || address->kind != NodeKind::Subsubsection) {
        return EditResult::Fail(FailureReason::InvalidTarget, "subsubsection not found");
    }
...
EditResult ProjectController::InsertParagraph(const NodeId& parent, const QString& text) {
...
EditResult ProjectController::InsertParagraphAfter(const NodeId& anchor, const QString& text) {
    ...
    if (!point) {
        if (session_->state().document().body().sections.empty()) {
            ...
            return EditResult::Fail(FailureReason::InvalidTarget, "insertion anchor not found");
        }
    }
...
EditResult ProjectController::InsertEquation(const NodeId& parent, const QString& math, bool numbered,
                                             const QString& label) {
...
EditResult ProjectController::InsertEquationAfter(const NodeId& anchor, const QString& math, bool numbered,
                                                  const QString& label) {
    auto point = ResolveInsertionPoint(anchor);
    if (!point) {
        return EditResult::Fail(FailureReason::InvalidTarget, "insertion anchor not found");
...
EditResult ProjectController::InsertFigure(const NodeId& parent, const QString& image_path) {
...
EditResult ProjectController::InsertFigureAfter(const NodeId& anchor, const QString& image_path) {
    auto point = ResolveInsertionPoint(anchor);
    if (!point) {
        return EditResult::Fail(FailureReason::InvalidTarget, "insertion anchor not found");
    }
    auto result = session_->InsertFigureFromSource(ToStd(image_path), point->parent, point->index);
    if (result.status == EditStatus::Applied)
        EmitDocumentChanged();
    return result;
}
...
EditResult ProjectController::EditParagraphRich(const NodeId& paragraph, const InlineContent& content) {
...
EditResult ProjectController::EditEquation(const NodeId& equation, const QString& math, bool numbered,
                                           const QString& label) {
...
EditResult ProjectController::RenameSection(const NodeId& section, const QString& title) {
...
EditResult ProjectController::RenameSubsection(const NodeId& subsection, const QString& title) {
...
EditResult ProjectController::EditCaption(const NodeId& block, const QString& caption) {
...
EditResult ProjectController::EditFigureSpan(const NodeId& figure, bool double_column) {
    EditFigureSpanPayload payload;
    payload.figure = figure;
    payload.span = double_column ? FigureSpan::DoubleColumn : FigureSpan::SingleColumn;
    return ExecuteAndNotify(std::move(payload));
}
...
EditResult ProjectController::MoveNodeAfter(const NodeId& node, const NodeId& anchor) {
    if (node == anchor) {
    ...
    if (!source || !target) {
        return EditResult::Fail(FailureReason::InvalidTarget, "move target not found");
    }
    ...
    if (source->block) {
        ...
        if (target->block) {
            ...
            case NodeKind::Subsection:
                anchor_parent = &doc.body().sections[*target->section].subsections[*target->subsection].id;
                break;
            default:
            ...
            if (source->section == target->section && source->subsection == target->subsection &&
                source->subsubsection == target->subsubsection && *source->block < *target->block) {
                --*index;
            }
        } else {
    ...
    if (source->kind == NodeKind::Subsubsection) {
        ...
        const auto& subsubsections =
            doc.body().sections[*source->section].subsections[*source->subsection].subsubsections;
        // 位于同级 subsubsection 之后时：紧跟其后。否则追加到末尾。
        const size_t to = target->kind == NodeKind::Subsubsection ? *target->subsubsection + 1 : subsubsections.size();
...
EditResult ProjectController::MoveNode(const NodeId& node, int direction) {
    if (direction != -1 && direction != 1) {
        return EditResult::Fail(FailureReason::ConstraintViolation, "direction must be -1 or +1");
    }
    const auto& sections = session_->state().document().body().sections;
    for (size_t si = 0; si < sections.size(); ++si) {
        if (sections[si].id == node) {
            const auto destination = static_cast<long long>(si) + direction;
            if (destination < 0 || destination >= static_cast<long long>(sections.size())) {
                return EditResult::Fail(FailureReason::InvalidTarget, "section is already at the edge");
            }
            MoveSectionPayload payload;
            payload.from = si;
        ...
        for (size_t bi = 0; bi < sections[si].blocks.size(); ++bi) {
            const auto id = std::visit([](const auto& block) { return block.id; }, sections[si].blocks[bi]);
            if (id == node) {
                const auto destination = static_cast<long long>(bi) + direction;
                if (destination < 0 || destination >= static_cast<long long>(sections[si].blocks.size())) {
                    return EditResult::Fail(FailureReason::InvalidTarget, "block is already at the edge");
                }
                MoveBlockPayload payload;
                payload.node = node;
        ...
        for (size_t ui = 0; ui < sections[si].subsections.size(); ++ui) {
            ...
            if (subsection.id == node) {
                ...
                if (destination < 0 || destination >= static_cast<long long>(sections[si].subsections.size())) {
                    return EditResult::Fail(FailureReason::InvalidTarget, "subsection is already at the edge");
                }
                MoveSubsectionPayload payload;
            ...
            for (size_t bi = 0; bi < subsection.blocks.size(); ++bi) {
                const auto id = std::visit([](const auto& block) { return block.id; }, subsection.blocks[bi]);
                if (id != node)
                    continue;
                const auto destination = static_cast<long long>(bi) + direction;
                if (destination < 0 || destination >= static_cast<long long>(subsection.blocks.size())) {
                    return EditResult::Fail(FailureReason::InvalidTarget, "block is already at the edge");
                }
                MoveBlockPayload payload;
                payload.node = node;
...
void ProjectController::CancelBuild() {
    session_->CancelBuild();
}

BibliographyImportResult ProjectController::ImportBibliographyText(const QString& bibtex) {
...
CitationSearchResult ProjectController::SearchCitations(const QString& query) const {
    return session_->SearchCitations(ToStd(query));
}

std::shared_ptr<const pf::CitationNumberResolver> ProjectController::CitationNumbers() const {
    if (!session_)
        return nullptr;
    return std::make_shared<const pf::CitationNumberResolver>(
        pf::CitationNumberResolver::Build(session_->state().document(), session_->bibliography()));
}
