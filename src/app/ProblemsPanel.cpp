...
ProblemsPanel::ProblemsPanel(QWidget* parent) : QWidget(parent) {
    ...
    count_label_->setStyleSheet(QString("color: %1; font-size: 8pt;").arg(theme::kSecondaryText));
    ...
    for (const char* f : {"All", "Errors", "Warnings"}) {
        ...
        chip->setStyleSheet(QString("QPushButton { border: none; color: %1; font-size: 8pt; padding: 1px 8px;"
    ...
    tabs_->setStyleSheet(QString("QTabBar::tab { background: transparent; color: %1; padding: 4px 12px;"
...
QWidget* ProblemsPanel::BuildProblemsTab() {
    ...
    list_->setStyleSheet(QString("QListWidget { background: transparent; border: none; }"
...
QWidget* ProblemsPanel::BuildLogTab() {
    ...
    auto add_tool_button = [&](const QString& text, auto slot) {
        ...
        button->setStyleSheet(QString("QPushButton { border: none; color: %1; font-size: 8pt; padding: 1px 8px; }"
    ...
    add_tool_button("Copy", [this]() { QApplication::clipboard()->setText(log_view_->toPlainText()); });
    ...
    auto_scroll_button_ = add_tool_button("Auto Scroll: On", [this]() {
        ...
        auto_scroll_button_->setText(on ? "Auto Scroll: On" : "Auto Scroll: Off");
    });
...
void ProblemsPanel::SetDiagnostics(const std::vector<Diagnostic>& diagnostics) {
...
QString ProblemsPanel::LocationText(const pf::Diagnostic& d) const {
    ...
    if (d.location.has_block_location()) {
        if (!d.location.label.empty())
            parts << ToQ(d.location.label);
    }
    if (d.location.has_file_location()) {
        parts << QString("%1:%2").arg(ToQ(d.location.file), QString::number(*d.location.line));
    }
    if (d.location.kind == DiagnosticLocationKind::CitationKey && !d.location.citation_key.empty()) {
        parts << QString("[%1]").arg(ToQ(d.location.citation_key));
    }
    if (parts.isEmpty())
        return ToQ(ToString(d.source));
    return parts.join(" \u00b7 ");
}
...
void ProblemsPanel::Refilter() {
    ...
    if (state_ != ProblemsState::Done) {
        const QString text = state_ == ProblemsState::Building ? QStringLiteral("Building\u2026")
                                                               : QStringLiteral("No build diagnostics available.");
    ...
    if (diagnostics_.empty()) {
        auto* item = new QListWidgetItem(list_);
        item->setText(QString::fromUtf8("\xE2\x9C\x94") + " No problems detected.");
        item->setData(Qt::UserRole, -1);
        item->setFlags(Qt::NoItemFlags);
    ...
    for (size_t i = 0; i < diagnostics_.size(); ++i) {
        ...
        const char* severity = d.severity == DiagnosticSeverity::Error     ? "error"
                               : d.severity == DiagnosticSeverity::Warning ? "warning"
                                                                           : "info";
        if (filter_ != "all" && filter_ != severity)
            continue;
        QString text = QString("%1  %2\n    %3 \u00b7 %4")
                           .arg(EventIcon(d.severity), ToQ(d.message), ToQ(SeverityWord(d.severity)), LocationText(d));
        auto* item = new QListWidgetItem(list_);
        item->setText(text);
        item->setData(Qt::UserRole, static_cast<int>(i));
        const char* color = d.severity == DiagnosticSeverity::Error     ? theme::kError
                            : d.severity == DiagnosticSeverity::Warning ? theme::kWarning
...
void ProblemsPanel::AppendEvent(const pf::BuildEvent& event) {
    ...
    QString text;
    if (event.type == BuildEventType::StdOut || event.type == BuildEventType::StdErr) {
        ...
        if (last_stream_ != stream) {
            if (log_needs_newline_)
                text += "\n";
            else if (!last_stream_.empty())
                text += "\n";
        ...
        if (log_needs_newline_)
            text += "\n";
        text += QString("%1 %2").arg(ToQ(FormatBuildTimestamp(event.timestamp_ms).c_str()), ToQ(event.message));
    }
    events_.push_back(event);

    const bool was_at_bottom = !log_view_->verticalScrollBar() ||
                               log_view_->verticalScrollBar()->value() >= log_view_->verticalScrollBar()->maximum();
