...
void SettleBuilds(MainWindow& window, int timeout_ms = 30000) {
    ...
    while (timer.elapsed() < timeout_ms) {
        ...
        QThread::msleep(5);
        if (pf::BuildPhase::Idle == session.build_phase() && !session.HasPendingApplicationEvents()) {
            QCoreApplication::processEvents(QEventLoop::AllEvents, 20);
...
struct RowEditor {
    ...
    QTextEdit* AsRich() const {
        return rich;
    }
    QPlainTextEdit* AsPlain() const {
        return plain;
    }
    int LineSpacing() const {
        return plain ? plain->fontMetrics().lineSpacing() : rich->fontMetrics().lineSpacing();
    }
    ...
    template <typename... Args> void keyClicks(Args... args) {
        QTest::keyClicks(plain ? static_cast<QWidget*>(plain) : static_cast<QWidget*>(rich), args...);
    }
    bool operator==(const RowEditor& other) const {
...
RowEditor FindRow(MainWindow& window, const QString& focus_key) {
    ...
    for (QWidget* widget : window.findChildren<QWidget*>()) {
        if (!widget->isVisible())
            continue;
        if (widget->property("row_focus_key").toString() != focus_key)
            continue;
...
QString WriteMultiPagePdf(const QString& path, int page_count) {
    ...
    objects.push_back("<< /Type /Pages /Kids [" + kids + "] /Count " + std::to_string(page_count) + " >>");
    for (int i = 0; i < page_count; ++i) {
...
int main(int argc, char* argv[]) {
    ...
    Check(window.controller()->NewProject(QString::fromStdString(dir.string())), "create project");
    // 保存是异步的（snapshot -> save worker），因此要 flush 以确保测试重新打开该
    // 目录之前 project.paper 已落盘。
    window.controller()->Save();
    window.controller()->FlushSaves();
    Spin(100);
    Check(window.OpenProjectDir(QString::fromStdString(dir.string())), "open project shows the workspace");
    Spin(200);
    ...
    Check(title_after_idle == title, "no rebuild happens while a row has uncommitted input");
    Check(title_after_idle != nullptr && title_after_idle.toPlainText() == typed,
          "typed title text survives 2s of idle time");
    Check(QApplication::focusWidget() == title.get(), "focus is still in the title row after idle time");

    const auto& fm_before = window.controller()->session().state().document().front_matter();
    ...
    Check(pf::InlineToPlainText(window.controller()->session().state().document().front_matter().title) ==
              typed.toStdString(),
          "Enter commits the typed title to the document");

    RowEditor title_after_commit = FindRow(window, "front:title");
    Check(title_after_commit != nullptr && title_after_commit.toPlainText() == typed,
    ...
    if (abstract_row) {
        ...
        Check(abstract_row.toPlainText() == abstract_text, "typing lands in the abstract row");

        Spin(2000);
        RowEditor abstract_idle = FindRow(window, "front:abstract");
        Check(abstract_idle == abstract_row, "multi-line row is not rebuilt while being edited");
        Check(abstract_idle != nullptr && abstract_idle.toPlainText() == abstract_text,
              "multi-line abstract survives idle time");

        // 失焦时提交它（title 行取得焦点）。
        RowEditor title_row = FindRow(window, "front:title");
        if (title_row)
            title_row.setFocus(Qt::MouseFocusReason);
        Spin(400);
        const auto& fm = window.controller()->session().state().document().front_matter();
        Check(fm.abstract_text.has_value() && pf::InlineToPlainText(*fm.abstract_text) == abstract_text.toStdString(),
              "focus-out commits the abstract to the document");
    }

    // ---- 4. 行高度适应其文本；只有窗格滚动 ----
    auto section = window.controller()->InsertSection(QStringLiteral("Body"));
    auto inserted = window.controller()->InsertParagraph(section.created_node, QStringLiteral("seed"));
    Spin(300);
    const QString paragraph_key = QString::fromStdString(inserted.created_node.value());
    RowEditor paragraph;
    for (QWidget* widget : window.findChildren<QWidget*>()) {
        if (!widget->isVisible())
            continue;
    ...
    if (paragraph) {
        ...
        Spin(60);
        Check(paragraph.VerticalScrollBarPolicy() == Qt::ScrollBarAlwaysOff, "no scrollbar inside a block");
        ...
        Check(tall_height > one_line_height + 20, "row height grows with the wrapped text");
        Check(paragraph.document()->blockCount() == 1, "wrapping did not invent extra blocks");

        // 文本必须真正可见：document 排布后的高度必须能容纳在 widget 内。
        const QRectF last =
            paragraph.document()->documentLayout()->blockBoundingRect(paragraph.document()->lastBlock());
        Check(last.bottom() <= paragraph.height(), "widget is at least as tall as the laid-out text");
        ...
        // 可见。此处独立于 widget 进行测量：每个 block 至少需要占一行。
        {
            QStringList lines;
            for (int i = 0; i < 20; ++i) {
                lines << QStringLiteral("Line %1 of a pasted multi-line abstract that is "
            ...
            std::cout << "  pasted: blocks=" << blocks << " height=" << pasted_height
                      << " line_spacing=" << paragraph.LineSpacing() << "\n";
            Check(blocks == 20, "pasted text kept its 20 lines");
            Check(pasted_height >= blocks * paragraph.LineSpacing(), "row height covers every pasted line");
        }
        ...
        if (paragraph)
            paragraph.SetPlainText(QStringLiteral("short"));
        Spin(250);
        Check(paragraph && paragraph.height() < tall_height, "row height shrinks again when the text is cleared");

        // 整个窗格恰好由一个滚动条提供服务。
        int visible_pane_scrollbars = 0;
        for (QScrollArea* area : window.findChildren<QScrollArea*>()) {
            if (area->isVisible() && area->verticalScrollBarPolicy() != Qt::ScrollBarAlwaysOff) {
    ...
    // ---- 5. 预览缩放跟随鼠标滚轮 ----
    {
        ...
        if (preview) {
            ...
            QWidget* viewport = preview->findChild<QScrollArea*>()->viewport();
            const QPointF centre(viewport->width() / 2.0, viewport->height() / 2.0);
            QWheelEvent up(centre, viewport->mapToGlobal(centre.toPoint()), QPoint(0, 0), QPoint(0, 120), Qt::NoButton,
            ...
            QWheelEvent down(centre, viewport->mapToGlobal(centre.toPoint()), QPoint(0, 0), QPoint(0, -120),
                             Qt::NoButton, Qt::NoModifier, Qt::NoScrollPhase, false);
            QApplication::sendEvent(viewport, &down);
            Spin(80);
            Check(preview->zoom() < zoomed_in, "wheel down zooms the preview out");

            preview->SetZoom(100.0);
            Check(preview->zoom() <= PdfPreview::kMaxZoom, "zoom is clamped to the maximum");
            preview->SetZoom(0.0001);
            Check(preview->zoom() >= PdfPreview::kMinZoom, "zoom is clamped to the minimum");
        }
    }
    ...
    // ---- 6. 硬换行的粘贴内容会按块宽度重新排版 ----
    {
        ...
        if (RowEditor row = abstract_row_fn()) {
            ...
            // 中途被断开。
            const QString hard = QStringLiteral("The success of deep learning in infrared small\n"
            ...
            if (pasted) {
                ...
                std::cout << "  reflow: lines=" << shown.count(QLatin1Char('\n')) + 1 << "\n";
                Check(shown.count(QLatin1Char('\n')) == 0, "pasted hard wraps became soft");
                Check(shown.contains(QStringLiteral("infrared small target detection")),
                ...
                auto max_line = [](RowEditor edit) {
                    ...
                    for (QTextBlock b = edit.document()->begin(); b.isValid(); b = b.next()) {
                        if (b.layout()) {
                            widest = qMax(widest, b.layout()->boundingRect().width());
                        }
                    }
                    return widest;
                };
                ...
                std::cout << "  reflow: narrow_line=" << narrow << " wide_line=" << wide << "\n";
                Check(wide > narrow + 20.0, "re-flowed prose follows the block width");
                window.resize(window.width() - 300, window.height());
                Spin(300);
            }
            ...
            Spin(250);
            const auto& front = window.controller()->session().state().document().front_matter();
            const std::string stored =
                front.abstract_text ? pf::InlineToPlainText(*front.abstract_text) : std::string();
            Check(stored.find("infrared small target detection") != std::string::npos,
                  "committed text keeps the re-flowed sentence");
            Check(stored.find('\n') == std::string::npos, "committed text has no stray hard wraps");
        }
    }
    ...
    // ---- 7. 每个 block 都带有悬停插入入口 ----
    {
        ...
        auto paragraphs_in_body = [&]() {
            ...
            for (const auto& section : doc.body().sections) {
                for (const auto& block : section.blocks) {
                    if (std::get_if<pf::Paragraph>(&block))
                        ++count;
                }
            }
            return count;
        ...
        for (QWidget* gap : before) {
            ...
            Check(!gap->property("gap_anchor").toString().isEmpty(), "each gap knows which block it follows");
        }
        ...
        if (target_gap && editor) {
            ...
            QString got_kind;
            QString got_anchor;
            QObject::connect(editor, &BlockEditor::InsertBlockRequested, [&](QString kind, QString anchor) {
                got_kind = kind;
                got_anchor = anchor;
            });
            ...
            QObject::connect(picker, &QTimer::timeout, [&]() {
                ...
                for (QAction* action : popup->actions()) {
                    if (action->isEnabled() && action->data().toString() == QStringLiteral("text")) {
                        preferred = action;
                        break;
                    }
                }
            ...
            Check(got_kind == QStringLiteral("text"), "the gap menu offers block kinds");
            Check(got_anchor == paragraph_key, "the gap inserts at its own position");
            Check(paragraphs_in_body() == paragraphs_before + 1, "the gap button really inserted a block");
            Check(gaps().size() == before.size() + 1, "the new block got its own gap");
        }
    }
    ...
    // ---- 8. 卡片菜单可重排 block ----
    {
        ...
        auto second = ctl->InsertParagraph(sec.created_node, QStringLiteral("beta"));
        // 只有本节内的段落才算数。
        auto order = [&]() {
            QStringList texts;
            const auto& doc = window.controller()->session().state().document();
            for (const auto& s : doc.body().sections) {
                if (s.id != sec.created_node)
                    continue;
        ...
        Spin(400);
        Check(order() == QStringLiteral("alpha,beta"), "paragraphs start in order");
        ...
        for (QWidget* w : window.findChildren<QWidget*>()) {
            ...
            if (frame && frame->objectName() == QStringLiteral("blockCard") &&
                frame->property("row_node").toString() == QString::fromStdString(second.created_node.value())) {
                card = frame;
            }
        }
        ...
        if (more) {
            std::function<QAction*(QMenu*, const QString&)> find_action = [&](QMenu* menu,
                                                                              const QString& text) -> QAction* {
                for (QAction* action : menu->actions()) {
                    if (action->text() == text)
                        return action;
                    if (action->menu()) {
                        if (QAction* nested = find_action(action->menu(), text)) {
                            return nested;
                        }
                    }
                }
            ...
            QObject::connect(picker, &QTimer::timeout, [&]() {
                ...
                if (!popup)
                    return;
                if (QAction* action = find_action(popup, QStringLiteral("Move Up"))) {
            ...
            picker->deleteLater();
            Spin(400);
            std::cout << "  order after Move Up=" << order().toStdString() << "\n";
            Check(order() == QStringLiteral("beta,alpha"), "Move Up reorders the document");
        }
    }
    ...
    // ---- 9. 预览依次显示每一页 ----
    {
        ...
        if (preview) {
            const QString pdf = WriteMultiPagePdf(QString::fromStdString((dir / "multi.pdf").string()), 3);
            ...
            auto sheets_now = [&]() {
                ...
                if (!column || !column->layout())
                    return found;
                for (QLabel* label : column->findChildren<QLabel*>(QStringLiteral("pdfPageSheet"))) {
            ...
            std::cout << "  sheets=" << sheets.size() << " page_h=" << (sheets.empty() ? -1 : sheets.front()->height())
                      << "\n";
            Check(sheets.size() == 3, "one sheet per page");
            if (sheets.size() == 3) {
                const int seam = sheets[1]->y() - (sheets[0]->y() + sheets[0]->height());
                const int seam2 = sheets[2]->y() - (sheets[1]->y() + sheets[1]->height());
                std::cout << "  seams=" << seam << "," << seam2 << "px\n";
                Check(seam >= 8 && seam <= 24 && seam2 >= 8 && seam2 <= 24, "pages are stacked with a visible seam");
            ...
            const bool last_rendered = scrolled.size() == 3 && !scrolled[2]->pixmap(Qt::ReturnByValue).isNull();
            std::cout << "  visible at bottom=" << preview->visiblePage() << " sheets=" << scrolled.size()
                      << " last has raster=" << (last_rendered ? 1 : 0) << "\n";
            Check(preview->visiblePage() == 3, "scrolling reaches the last page");
    ...
    // ---- 10. 在 GUI 中把作者关联到机构 ----
    {
        ...
        for (QToolButton* button : window.findChildren<QToolButton*>()) {
            if (button->objectName() == QStringLiteral("authorAffiliationPicker") && button->isVisible()) {
                pickers.push_back(button);
            }
        }
        std::cout << "  pickers=" << pickers.size() << "\n";
        Check(pickers.size() == 2, "one institution picker per author");
        Check(!pickers.empty() && pickers[0]->text() == QStringLiteral("link…"), "an unlinked author says so");

        const auto affiliations = ctl->session().state().document().front_matter().affiliations;
        ...
        if (pickers.size() == 2 && affiliations.size() == 2) {
            ...
            Check(ctl->SetAuthorAffiliation(0, affiliations[1].id, true).status == pf::EditStatus::Applied,
                  "link author 1 to institution 2");
            Spin(400);
            const auto& authors = ctl->session().state().document().front_matter().authors;
            Check(authors[0].affiliations.size() == 1 && authors[0].affiliations[0] == affiliations[1].id,
                  "the binding is stored on the author");
            Check(authors[1].affiliations.empty(), "other authors are untouched");

            // Authors 行会显示该标记，面板也会显示它。
            RowEditor authors_row = FindRow(window, "front:authors");
            Check(authors_row != nullptr, "authors row present");
            if (authors_row) {
                std::cout << "  authors row='" << authors_row.toPlainText().toStdString() << "'\n";
                Check(authors_row.toPlainText().contains(QString::fromUtf8("\u00b2")),
                      "the row shows the institution number");
            }

            Check(ctl->SetAuthorAffiliation(0, affiliations[1].id, false).status == pf::EditStatus::Applied,
                  "unlink again");
            Spin(300);
            const auto& after = ctl->session().state().document().front_matter();
            Check(after.authors[0].affiliations.empty(), "unlinking removes the binding");
        }
    }
    ...
    // ---- 11. 把 block 拖放到 gap 上可重排 document ----
    {
        ...
        Check(order() == QStringLiteral("alpha,beta,gamma"), "blocks start in order");
        ...
        for (QObject* child : window.findChildren<QObject*>()) {
            if (child->inherits("QWidget") &&
                child->property("row_node").toString() == QString::fromStdString(gamma.created_node.value())) {
        ...
        Check(ctl->MoveNodeAfter(gamma.created_node, alpha.created_node).status == pf::EditStatus::Applied,
              "MoveNodeAfter is accepted");
        Spin(500);
        std::cout << "  order after controller move=" << order().toStdString() << "\n";
        Check(order() == QStringLiteral("alpha,gamma,beta"), "MoveNodeAfter reorders the document");

        // 然后通过对「beta」之后的 gap 执行真实拖放来做同样的事：gamma 必须回到
        // 末尾。
        QWidget* target_gap = nullptr;
        for (QWidget* w : window.findChildren<QWidget*>()) {
            if (w->property("gap_anchor").toString() == QString::fromStdString(beta.created_node.value()) &&
                w->isVisible()) {
                target_gap = w;
            }
        }
        ...
        if (target_gap) {
            ...
            const QString gamma_id = QString::fromStdString(gamma.created_node.value());
            const bool invoked = QMetaObject::invokeMethod(target_gap, "applyDrop", Q_ARG(QString, gamma_id));
            Check(invoked, "the gap accepts a dropped block");
            Spin(500);
            std::cout << "  order after drop=" << order().toStdString() << "\n";
            Check(order() == QStringLiteral("alpha,beta,gamma"), "the dragged block lands where it was dropped");
        }
    }
