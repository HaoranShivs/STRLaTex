...
int main(int argc, char* argv[]) {
    ...
    QTimer::singleShot(300, [&]() {
        ...
        QString dir = existing.isEmpty() ? QStringLiteral("/tmp/pf-ui-demo") : existing;
        if (!existing.isEmpty()) {
            ...
            QTimer::singleShot(1800, [&window]() {
                ...
                auto find_row = [&window](const QString& key) -> QPlainTextEdit* {
                    for (QPlainTextEdit* edit : window.findChildren<QPlainTextEdit*>()) {
                        if (edit->isVisible() && edit->property("row_focus_key").toString() == key) {
                            return edit;
                        }
                    }
                    return nullptr;
        ...
        controller->SetAffiliationsText("School of Information and Communication Engineering, University "
                                        "One; Institute of Optoelectronics, Institute Two");
        controller->SetAuthorsText("Tanran Shi\u00b9\u00b2 · Author B\u00b9 · Author C\u00b2");
        controller->SetAbstract("Infrared small target detection has attracted considerable "
        ...
        // 第 1 张与第 2 张纸之间的接缝。
        for (int i = 0; i < 26; ++i) {
            controller->InsertParagraph(sec.created_node, QString("Paragraph %1. Infrared small target detection under "
        ...
        controller->InsertParagraph(sec.created_node, "Infrared small target detection has received considerable "
        ...
        {
            const pf::EditResult rich_para = controller->InsertParagraphAfter(sec.created_node, QString());
            if (rich_para.status == pf::EditStatus::Applied && !rich_para.created_node.empty()) {
                ...
                rich.push_back(frac);
                rich.push_back(pf::TextRun{" drives the refinement stage down the image sequence.", 0});
                controller->EditParagraphRich(rich_para.created_node, rich);
            }
        }
        controller->ImportBibliographyText(QString("@article{wang2025, author={W. Wang}, title={Infrared Target "
                                                   "Detection}, year={2025}}\n"
        ...
        QTimer::singleShot(400, [&window, controller, &failures]() {
            ...
            QObject::connect(controller, &ProjectController::previewUpdated,
                             [&window, ctl](const pf::PreviewUpdate& update) {
                                 std::cout << "[ EVNT ] previewUpdated success=" << update.success << "\n";
                                 if (update.success) {
                                 ...
                                 QMetaObject::invokeMethod(
                                     ctl, [ctl]() { ctl->RequestBuild(true); }, Qt::QueuedConnection);
                             });
