...
class BlockEditor : public QWidget {
  ...
  public:
    ...
    void SetMaxHeadingDepth(int depth) {
        max_heading_depth_ = depth;
    }
    ...
    void SetCitationNumbers(std::shared_ptr<const pf::CitationNumberResolver> numbers);
  ...
  signals:
    ...
    void ParagraphContentEdited(const QString& node_id, const InlineContent& content);
    void EquationEdited(const QString& node_id, const QString& math, bool numbered, const QString& label);
    ...
    // 作者 <-> 机构绑定从图形面板发生变化。
    void AuthorAffiliationToggled(int author_index, const QString& affiliation_id, bool linked);
  ...
  public:
    ...
    bool InsertCitationIntoParagraph(const QString& node_id, const QString& citation_key, int insert_offset = -1);
  ...
  private:
    ...
    QWidget* MakeTextCard(const QString& node_id, const InlineContent& content, const QString& outline_key);
    // Equation 行：LaTeX 源码 + 预览 + 编号 + 标签（设计 §4）。
    QWidget* MakeEquationCard(const QString& node_id, const pf::EquationBlock& equation, const QString& outline_key);
    ...
    void AppendBlocks(const std::vector<pf::Block>& blocks, const QString& outline_key);
    ...
    QWidget* MakeCard(const QString& node_id, const QString& kind, const QString& commit_role, bool header_inline);
    // 编辑器窗格宽于正文栏时，使正文栏保持居中并限制在主题的内容宽度内。
    void CenterContentColumn();
    void AddEditorToCard(QWidget* card, QPlainTextEdit* edit);
    QPlainTextEdit* NewEditor(QWidget* card, const QString& text, int min_lines, theme::BlockVisualRole role,
